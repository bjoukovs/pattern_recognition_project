...
if nargin < 1 | isempty(K),
  W = prmapping(mfilename,alf,prec);
  W = setname(W,'IKPCA');
  return
end
...
if ismapping(K),
  ...
  W = prmapping(mfilename,'trained',{QL,pars{2},L,sig,pars{5}},[],m,length(J));
  W = setname(W,'IKPCA');
  return
end
...
W = prmapping(mfilename,'trained',{QL,Kme,L,sig,prec},[],m,sum(sig));
W = setname(W,'IKPCA');
return
